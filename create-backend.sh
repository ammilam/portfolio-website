...
terraform {
  backend "\"gcs\""{
    bucket      = "\"${PROJECT}-portfolio-website-tfstate\""
    prefix      = "\"portfolio\""
...
if [[ -z $(gsutil ls|grep ${PROJECT}-portfolio-website-tfstate) ]]
then
    gsutil mb gs://${PROJECT}-portfolio-website-tfstate
fi
terraform fmt --recursive
echo "Your remote state bucket is ${PROJECT}-portfolio-website-tfstate"